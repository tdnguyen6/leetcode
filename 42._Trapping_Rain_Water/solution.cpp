...
int calTrappedRain(int l, int r, int level, vector<int> height)
{
    int rain = 0;
    for (int i = l + 1; i < r; i++)
    {
        rain += height[level] - height[i];
    }
    return rain;
}
...
int solve(vector<int> height)
{
    ...
    for (int i = 1; i < (int)height.size(); i++)
    {
        ...
        if (height[i] >= height[left])
        {
            cout << "Left: " << left << " i: " << i;
            if (i - left > 1)
                sum += calTrappedRain(left, i, left, height);
            left = i;
        }
        if (height[j] > height[right])
        {
            cout << "j: "
                 << " Right: " << right << j;
            if (right - j > 1)
                sum += calTrappedRain(j, right, right, height);
            right = j;
        }
        cout << " Sum: " << sum << '\n';
    }
